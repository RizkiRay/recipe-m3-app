...
def clean_title_and_parse(r):
    ...
    title = ''
    for line in lines:
        if line and len(line) >= 4 and not line.lower().startswith(('follow', 'original', 'edited', 'http', '#', 'see translation', 'view', author.lower(), 'liked by', 'likes', 'reply', 'this page')):
            title = line
            break
...
async def fetch_reel_robust(sc):
            ...
            async with websockets.connect(ws_url, max_size=10**8) as ws:
                ...
                while True:
                    ...
                    if msg.get('id') == 2:
                        ...
                        if tab_id:
                            urllib.request.urlopen(urllib.request.Request(f'http://localhost:9222/json/close/{tab_id}', method='PUT'))
                            tab_id = None
                            
                        # If rate-limited or blank, retry
                        if not val.get('caption') or 'this page isn' in val.get('caption', '').lower():
                        ...
                        if parsed:
                            insert_to_db(parsed)
                            print(f"✓ [{sc}] Saved: {parsed['title']} ({parsed['chef']})")
                            return True
                        break
...
async def main():
    with open('/tmp/remaining_to_252.json') as f:
        remaining = json.load(f)
        
    print(f"Starting robust single-lane scrape for {len(remaining)} remaining reels...")
    
    success_count = 0
    for idx, sc in enumerate(remaining):
        ok = await fetch_reel_robust(sc)
        if ok:
            success_count += 1
        # Gentle delay between requests to completely avoid Instagram rate limits
        await asyncio.sleep(1.2)
        if (idx + 1) % 10 == 0:
            print(f"--- Progress: {idx+1}/{len(remaining)} processed (Saved: {success_count}) ---")
            
    print(f"Ingestion complete! Successfully added {success_count} recipes.")
